...
t_tuple *normal_at(t_object *s, t_tuple *world_point)
{
	...
	if(s->type == SPHERE)
	{
		...
		t_tuple *object_point = matrix_multiply_by_tuple(s->inverse, world_point);
		object_normal = substrctTuples(object_point, abs_point);
		free(abs_point);
		free(object_point);
	}
	t_matrix *trans = matrix_transpose(s->inverse);
